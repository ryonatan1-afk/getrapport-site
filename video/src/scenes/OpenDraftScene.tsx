...
export const OpenDraftScene = () => {
	...
	const emailSlide = spring({frame: frame - 60, fps, config: {damping: 16, stiffness: 100}});
	const cursorBlink = Math.floor(frame / 20) % 2 === 0;
	const clickPulse = spring({frame: frame - 30, fps, config: {damping: 8, stiffness: 200}});
	const emailClientOpacity = interpolate(frame, [90, 130], [0, 1], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'});
	const emailClientY = interpolate(
		spring({frame: frame - 90, fps, config: {damping: 16, stiffness: 100}}),
		[0, 1], [30, 0]
	);
